...
gggtable <- function(data) {
  ...
  if (ggplot2:::is.zero(legend_box)) { # ******
    ...
    if (position == "manual") {
      ...
      legend_box <- grid::editGrob(legend_box, # ******
        vp = grid::viewport(
          x = xpos, y = ypos, just = c(xjust, yjust), # ******
  ...
  if (!(tag_pos == "manual" || tag_pos %in% valid_pos)) {
    abort(glue(
      "plot.tag.position should be a coordinate or one of ",
  ...
  if (tag_pos == "manual") {
    ...
    plot_table <- gtable_add_grob(plot_table, tag_parent,
      name = "tag", t = 1,
    ...
    if (tag_pos == "topleft") {
      ...
      plot_table <- gtable::gtable_add_grob(plot_table, tag,
        name = "tag", # ******
        t = 1, l = 1, clip = "off"
      )
    } else if (tag_pos == "top") {
